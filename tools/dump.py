...
def dump_conv(
    ...
    return {
        ...
        "weight": weight,
        "bias": bias if len(node.input) > 2 else None,
        "kernel_size": kernel_size[0] if type == "conv1d" else kernel_size,
        "stride": stride[0] if type == "conv1d" else stride,
        "padding": padding,
        "dilation": dilation[0] if type == "conv1d" else dilation,
        "groups": groups,
        "out_shift": out_shift,
        "activation": activation if activation else None,
        "input_off": planner.get_offset(node.input[0]),
...
def dump_maxpool(
    ...
    return {
        ...
        "input_shape": input_shape,
        "output_shape": output_shape,
        "kernel_size": kernel_size[0] if type == "maxpool1d" else kernel_size,
        "stride": stride[0] if type == "maxpool1d" else stride,
        "padding": padding,
        "dilation": dilation[0] if type == "maxpool1d" else dilation,
        "out_shift": out_shift,
        "input_off": planner.get_offset(node.input[0]),
        "output_off": planner.get_offset(node.output[0]),
    }
