...
from model.dataset import load_gesture_data
from model.net import QuantGesture
...
def train(net: str, save_dir: Path, args: Namespace):
    ...
    train_loader, test_loader = load_gesture_data(args.dataset, args.batch_size)
    input_t = torch.randn(1, 3, 150).to(device)
    model = QuantGesture(num_classes=13).to(device)
...
def main():
    ...
    parser.add_argument("--dataset", type=str, default="/path/to/GestureDataset")
